...
/**
 * This class represents/implements a SECSItem with the SECS data type of <code>F4</code>
 * in an array form.
 * In this case it is an array of zero or more 32-bit floating point numbers.  From the Java side this data
...
public class F4ArraySECSItem extends SECSItem
{
    public float[] value = null;
    
    /**
     * This constructor creates a SECSItem that has a type of <code>F4</code> with 
     * the minimum number of length bytes required.
     * <br>
     * Note: It will be created with the number of length bytes required
     * based on the length (in elements) of the <code>float[]</code> provided.
     * The maximum array length allowed is <code>16777215</code> bytes(elements).
     * 
     * @param value an array of Java <code>float</code> to be assigned to this SECSItem
     */
    public F4ArraySECSItem(float[] value)
    {
        super(SECSItemFormatCode.F4, value.length * 4);
        this.value = value;
    }
    
    /**
     * This constructor creates a SECSItem that has a type of <code>F4</code>
     ...
     * length (in elements) of the <code>float[]</code> provided.
     * 
     ...
     */
    public F4ArraySECSItem(float[] value, SECSItemNumLengthBytes desiredNumberOfLengthBytes)
    {
        super(SECSItemFormatCode.F4, value.length * 4, desiredNumberOfLengthBytes);
        this.value = value;
    }
     ...
     */
    public F4ArraySECSItem(byte[] data, int itemOffset)
    {
        ...
        for (int i = 0, j = offset; i < value.length; i++, j+= 4)
        {
            value[i] = buf.getFloat(j);
        }
    }
    ...
    public byte[] toRawSECSItem()
    {
        ...
        {
            ByteBuffer bb = ByteBuffer.wrap(output, offset, value.length * 4);
            bb.order(ByteOrder.BIG_ENDIAN);
            bb.putFloat(i, value[j]);
        }
